...
import { getAccessToken } from "@sceneenglish/app-core/platform/context";
import {
...
type RoutePrefetcher = { prefetch?: (href: string) => void };
...
export function prefetchAllRoutes(qc: QueryClient, router: RoutePrefetcher) {
...
export function prefetchNavTarget(qc: QueryClient, router: RoutePrefetcher, href: string) {
  router.prefetch?.(href);
  prefetchRouteData(qc, href);
