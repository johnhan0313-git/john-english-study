...
import { useParams, useNavigate, usePlatform } from "../../../platform/context";
...
export default function ScenarioDetailPage() {
  ...
  const shadowSentenceRef = useRef<string>("");

  useEffect(() => {
    platformAudio.setOnEnded?.(() => setPlaying(false));
    platformAudio.setOnError?.(() => setPlaying(false));
    return () => {
      platformAudio.pause();
      platformAudio.setOnEnded?.(null);
      platformAudio.setOnError?.(null);
    };
  }, [platformAudio]);
  ...
  const toggleAudio = async () => {
    ...
    const url = api.getScenarioAudioUrl(Number(id));
    setPlaying(true);
    try {
      await platformAudio.play(url);
      platformAudio.setRate(playbackRate);
    } catch {
      setPlaying(false);
    }
  };
  ...
  const startRecording = async (sentence: string) => {
    shadowSentenceRef.current = sentence;
  ...
  const stopRecording = async () => {
    ...
    const result = await api.evaluateSpeaking(shadowSentenceRef.current, blob);
